...
```sh
go get -u 'github.com/TeamWarp/warp-go-sdk@v0.4.0'
```
...
params := FooParams{
	...
	Description: warphr.Null[string](),

	Point: warphr.F(warphr.Point{
		X: warphr.Int(0),
		Y: warphr.Int(1),

		// In cases where the API specifies a given type,
		// but you want to send something else, use `Raw`:
		Z: warphr.Raw[int64](0.01), // sends a float
	}),
}
```
...
See the [full list of request options](https://pkg.go.dev/github.com/TeamWarp/warp-go-sdk/option).
...
We also provide a helper `warphr.FileParam(reader io.Reader, filename string, contentType string)`
...
params := FooNewParams{
    ID:   warphr.F("id_xxxx"),
    Data: warphr.F(FooNewParamsData{
        FirstName: warphr.F("John"),
    }),
}
